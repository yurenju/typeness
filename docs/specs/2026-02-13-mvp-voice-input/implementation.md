...
**實作備註**
[技術障礙] Qwen3 即使在 system prompt 中加入 `/no_think`，仍會產生空的 `<think></think>` block。需在解碼後用 regex 去除。

[方向調整] `/no_think` 從 system prompt 移到 user message 開頭，這是 Qwen3 的正確用法。初版 prompt 過度精簡導致 LLM 省略實質內容（如「我想要買三個東西」），改用 few-shot 範例引導後效果顯著改善，贅字移除和列表格式化都正確。
